...
function _room(ctx,W,H){
  _r(ctx,0,0,W/_PX,_WH,'#1c1f3a');
  ctx.fillStyle='#252848';for(let x=0;x<W;x+=_PX*8)ctx.fillRect(x,0,1,_WH*_PX);
  _r(ctx,0,_WH-2,W/_PX,2,'#0f1125');
  const T=8;for(let ty=_WH;ty<H/_PX;ty+=T)for(let tx=0;tx<W/_PX;tx+=T){const alt=(Math.floor(tx/T)+Math.floor((ty-_WH)/T))%2;_r(ctx,tx,ty,T,T,alt?'#2a1f14':'#301e12');}
  ctx.strokeStyle='rgba(0,0,0,.35)';ctx.lineWidth=1;
  for(let x=0;x<W;x+=T*_PX){ctx.beginPath();ctx.moveTo(x,_WH*_PX);ctx.lineTo(x,H);ctx.stroke();}
  for(let y=_WH*_PX;y<H;y+=T*_PX){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke();}
}

// ── FURNITURE ──────────────────────────────────────────────────────────────────
function _desk(ctx,lx,ly,lc){
  _r(ctx,lx,ly,22,7,'#5c3d1e');_r(ctx,lx,ly,22,1,'#7a5228');_r(ctx,lx,ly,1,7,'#3a2208');
  _r(ctx,lx,ly+7,22,4,'#4a2f0f');_r(ctx,lx,ly+11,22,1,'#281600');
  _r(ctx,lx+2,ly+1,6,4,'rgba(235,242,255,.12)');_r(ctx,lx+10,ly+1,5,4,'rgba(235,242,255,.07)');
  _r(ctx,lx+3,ly+2,4,.5,lc,.4);_r(ctx,lx+3,ly+3.5,3,.5,lc,.28);
}
function _monitor(ctx,lx,ly,lc){
  const mx=lx+6,my=ly-12;
  _r(ctx,mx+3,my+11,5,2,'#1a1d2e');_r(ctx,mx+4,my+9,3,3,'#22253c');
  _r(ctx,mx,my,11,10,'#1e2235');_r(ctx,mx+1,my+1,9,8,'#060a10');
  ctx.globalAlpha=.22;ctx.fillStyle=lc;ctx.fillRect((mx+1)*_PX,(my+1)*_PX,9*_PX,8*_PX);ctx.globalAlpha=1;
  [[7,.8],[5,.55],[6,.45],[4,.38],[5,.48]].forEach(([w,a],i)=>{_r(ctx,mx+1.5,my+2+i*1.25,w,.7,lc,a);});
}
function _chair(ctx,lx,ly){
  _r(ctx,lx+1,ly-7,6,7,'#2a3d5c');_r(ctx,lx+1,ly-7,6,1,'#3a5278');
  _r(ctx,lx,ly,8,3,'#1f304d');_r(ctx,lx,ly,8,1,'#2d4470');
  _r(ctx,lx+1,ly+3,2,5,'#141e2e');_r(ctx,lx+5,ly+3,2,5,'#141e2e');
}
function _shelf(ctx,lx,ly){
  _r(ctx,lx,ly,5,20,'#4a2f0f');_r(ctx,lx,ly,5,1,'#6b4422');
  const bc=['#ef4444','#3b82f6','#f59e0b','#10b981','#8b5cf6','#ec4899','#06b6d4','#84cc16'];
  let ci=0;for(let row=0;row<3;row++){_r(ctx,lx,ly+1+row*6,5,1,'#3a2208');let bx=lx+.3;while(bx<lx+4.5){const bw=1+(ci%2?.5:0);_r(ctx,bx,ly+2+row*6,bw,4,bc[ci%bc.length]);_r(ctx,bx,ly+2+row*6,bw,.5,'rgba(255,255,255,.22)');bx+=bw+.3;ci++;}}
}
function _whiteboard(ctx,lx,ly){
  _r(ctx,lx,ly,18,14,'#7a868f');_r(ctx,lx+1,ly+1,16,12,'#edf2f7');_r(ctx,lx+1,ly+1,16,1,'rgba(255,255,255,.5)');
}
...
function _renderOffice(ctx,W,H,lessonId,frame,lc){
  ...
  // ── helper: standard desk+char setup ──
  function stdDesk(charState,bubble,propFn){
    _shelf(ctx,4,_WH+2);
    const dx=LW-32,dy=_WH+14;
    _desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
    if(propFn)propFn();
    const cx=dx+4,cy=dy+15;
    _char(ctx,cx,cy,charState,frame,lc);_bubble(ctx,cx,cy-10,bubble,lc);
  }

  switch(lessonId){
    case'l1_1':
      _shelf(ctx,4,_WH+2);
      _propTwoMonitors(ctx,22,_WH+10,frame);
      _char(ctx,LW/2-5,_WH+28,'idle',frame,lc);_bubble(ctx,LW/2-5,_WH+18,'SAS vs PySpark',lc);
      break;
    case'l1_2':
      stdDesk('reading','Datentypen',()=>_propTypeTable(ctx,8,_WH+5,lc,frame));break;
    case'l2_1':
      _shelf(ctx,4,_WH+2);
      _propFunnel(ctx,LW/2-8,_WH+8,lc,frame);
      stdDesk('pointing','WHERE filter',null);
      break;
    case'l3_1':
      stdDesk('reading','Sonderzeichen',()=>_propMagnifier(ctx,8,_WH+5,lc,frame));break;
    case'l3_2':
      _shelf(ctx,4,_WH+2);
      _propClean(ctx,8,_WH+8,lc,frame);
      {const dx=LW-32,dy=_WH+14;_char(ctx,dx+4,dy+15,'typing',frame,lc);_bubble(ctx,dx+4,dy+5,'Bereinigung',lc);}
      break;
    case'l3_3':
      stdDesk('pointing','DQ Report',()=>_propDQRows(ctx,8,_WH+6,lc,frame));break;
    case'l4_1':{
      const wbX=6,wbY=_WH+3;
      _propBars(ctx,wbX,wbY,lc,frame);
      const dx=LW-32,dy=_WH+14;
      _desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,wbX+20,_WH+24,'pointing',frame,lc);_bubble(ctx,wbX+20,_WH+14,'GROUP BY',lc);
      break;}
    case'l5_1':{
      const d1x=10,d2x=LW-34,dy=_WH+13;
      _desk(ctx,d1x,dy,'#f59e0b');_monitor(ctx,d1x,dy,'#f59e0b');_chair(ctx,d1x+7,dy+9);
      _desk(ctx,d2x,dy,'#38bdf8');_monitor(ctx,d2x,dy,'#38bdf8');_chair(ctx,d2x+7,dy+9);
      _propJoin(ctx,d1x,_WH+28,lc,frame,W);
      const t=((frame%220)/220),e=_ease(t);
      const cx=Math.round(d1x+26+e*(d2x-d1x-28)),cy=_WH+24;
      _char(ctx,cx,cy,(t>.06&&t<.94)?'walking':'idle',frame,lc);_bubble(ctx,cx,cy-10,'JOIN',lc);
      break;}
    case'l6_1':
      stdDesk('typing','DB Connect',()=>_propServer(ctx,6,_WH+5,lc,frame));break;
    case'l7_1':
      _shelf(ctx,4,_WH+2);_propGauge(ctx,6,_WH+5,lc,frame);
      {const dx=LW-32,dy=_WH+14;_desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,dx+4,dy+15,'typing',frame,lc);_bubble(ctx,dx+4,dy+5,'Performance',lc);}
      break;
    case'l8_1':
      _propPipeline(ctx,W,H,lc,frame);
      {const cy=_WH+34;_char(ctx,LW/2-5,cy,'walking',frame,lc);_bubble(ctx,LW/2-5,cy-10,'ETL Pipeline',lc);}
      break;
    case'l8_2':
      stdDesk('typing','MERGE',()=>_propMerge(ctx,6,_WH+8,lc,frame));break;
    case'l9_1':
      stdDesk('typing','SCD Typ 2',()=>_propSCD2(ctx,6,_WH+8,lc,frame));break;
    case'l10_1':
      _shelf(ctx,4,_WH+2);_propMeter(ctx,LW/2-11,_WH+6,lc,frame);
      {const dx=LW-32,dy=_WH+14;_desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,dx+4,dy+15,'reading',frame,lc);_bubble(ctx,dx+4,dy+5,'Smart Meter ⚡',lc);}
      break;
    case'l11_1':
      stdDesk('pointing','DQ Check',()=>_propDQDash(ctx,6,_WH+5,lc,frame));break;
    case'l12_1':
      _shelf(ctx,4,_WH+2);_propGDPR(ctx,10,_WH+5,lc,frame);
      {const dx=LW-32,dy=_WH+14;_desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,dx+4,dy+15,'idle',frame,lc);_bubble(ctx,dx+4,dy+5,'GDPR 🔒',lc);}
      break;
    case'l13_1':
      _shelf(ctx,4,_WH+2);_propMacro(ctx,6,_WH+5,lc,frame);
      {const dx=LW-32,dy=_WH+14;_desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,dx+4,dy+15,'typing',frame,lc);_bubble(ctx,dx+4,dy+5,'%MACRO',lc);}
      break;
    case'l14_1':
      stdDesk('pointing','KEEP/DROP',()=>_propDropKeep(ctx,8,_WH+6,lc,frame));break;
    case'l14_2':
      stdDesk('typing','CAT / CATX',()=>_propConcat(ctx,8,_WH+6,lc,frame));break;
    case'l14_3':
      stdDesk('reading','PRXCHANGE',()=>_propRegex(ctx,6,_WH+6,lc,frame));break;
    case'l15_1':
      _shelf(ctx,4,_WH+2);_propCalendar(ctx,LW/2-12,_WH+5,lc,frame);
      {const dx=LW-32,dy=_WH+14;_desk(ctx,dx,dy,lc);_monitor(ctx,dx,dy,lc);_chair(ctx,dx+7,dy+9);
      _char(ctx,dx+4,dy+15,'typing',frame,lc);_bubble(ctx,dx+4,dy+5,'Datum & Zeit',lc);}
      break;
    case'l16_1':
      _shelf(ctx,4,_WH+2);_propScales(ctx,LW/2-14,_WH+8,lc,frame);
      {const cx=LW/2,cy=_WH+30;_char(ctx,cx,cy,'idle',frame,lc);_bubble(ctx,cx,cy-10,'SQL Dialekte',lc);}
      break;
    case'l16_2':
      _shelf(ctx,4,_WH+2);_propMigrate(ctx,4,_WH+8,lc,frame);
      {const cy=_WH+26;_char(ctx,LW/2-5,cy,'walking',frame,lc);_bubble(ctx,LW/2-5,cy-10,'Migration →',lc);}
      break;
    default:
      stdDesk('typing','Coding...',null);break;
  }
}
