...
const LANG_COLOR = { SAS: '#f59e0b', PySpark: '#38bdf8' };
...
const DIFF_COLOR = { 1: '#10b981', 2: '#f59e0b', 3: '#f97316', 4: '#ef4444' };
...
function useCodeState(lesson) {
  ...
  useEffect(() => {
    const desc = lesson.exercise?.description || '';
    setSasCode(lesson.exercise?.sasSolution
      ? `/* ${desc}\n   Schreibe deinen SAS-Code hier: */\n\n` : '');
    setPyCode(lesson.exercise?.pysparkSolution
...
/** ProgressBars — dual SAS/PySpark progress bars for sidebar header. */
function ProgressBars({ sasDone, pyDone, sasPct, pyPct, totalLessons, langColorHex, mounted }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: 'var(--text3)', textTransform: 'uppercase', marginBottom: 12 }}>
        Lernfortschritt
      </div>
      {[
        { label: 'SAS',     color: '#f59e0b', done: sasDone, pct: sasPct },
        { label: 'PySpark', color: '#38bdf8', done: pyDone,  pct: pyPct  }
      ].map(({ label, color, done, pct }) => {
        const isActive = (label === 'SAS' && langColorHex === '#f59e0b') || (label === 'PySpark' && langColorHex === '#38bdf8');
        return (
          <div key={label} style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color, opacity: isActive ? 1 : 0.4 }}>{label}</span>
...
/** LessonItem — a single clickable lesson row inside a module group. */
function LessonItem({ lesson, index, isActive, isDone, langColorHex, mounted, staggerDelay, onClick }) {
  const diff = DIFFICULTY[lesson.id] || 1;
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      <div style={{
        ...
        background: isActive ? langColorHex + '25' : isDone ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.05)',
        border: '1px solid ' + (isActive ? langColorHex + '55' : isDone ? 'rgba(74,222,128,0.3)' : 'rgba(255,255,255,0.08)'),
        fontSize: 9, fontWeight: 800,
        color: isActive ? langColorHex : isDone ? '#4ade80' : 'var(--text3)',
        transition: 'all 0.2s'
      }}>
        {isDone ? '✓' : isActive ? '▶' : index + 1}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, lineHeight: 1.4 }}>{lesson.title}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
          <DiffStars id={lesson.id} size={6} />
          <span style={{ fontSize: 9, color: DIFF_COLOR[diff], fontWeight: 600, opacity: 0.7 }}>{DIFF_LABEL[diff]}</span>
        </div>
      </div>
    </div>
  );
...
/** ModuleGroup — collapsible accordion section for one module in the sidebar. */
function ModuleGroup({ mod, modIdx, activeLesson, completedLessons, langColorHex, mounted, collapsed, onToggle, onSelect }) {
  ...
  const sorted      = [...mod.lessons].sort((a, b) => (DIFFICULTY[a.id]||1) - (DIFFICULTY[b.id]||1));
  ...
  return (
    <div style={{
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          {modDone > 0 && (
            <div style={{
              ...
              justifyContent: 'center', fontSize: 8, fontWeight: 800, color: '#000'
            }}>{modDone}</div>
          )}
        </div>
        <span style={{
      ...
      <div style={{
        overflow: 'hidden',
        maxHeight: isCollapsed ? 0 : sorted.length * 62 + 'px',
        opacity: isCollapsed ? 0 : 1,
        transition: 'max-height 0.35s cubic-bezier(0.4,0,0.2,1), opacity 0.25s'
      }}>
        {sorted.map((lesson, lIdx) => (
          <LessonItem
            key={lesson.id}
            lesson={lesson}
            index={lIdx}
...
function SidebarPanel({ modules, activeLesson, activeModule, completedLessons, langColorHex, onSelect, totalLessons, doneCount, sasDone, pyDone, sasPct, pyPct }) {
  ...
  const toggleModule = (modId) => setCollapsed(prev => ({ ...prev, [modId]: !prev[modId] }));

  return (
    <aside style={{ width: 276, height: '100%', background: 'var(--sidebar-bg)', borderRight: '1px solid rgba(255,255,255,0.08)', overflowY: 'auto', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
      <ProgressBars
        sasDone={sasDone} pyDone={pyDone}
        sasPct={sasPct}   pyPct={pyPct}
...
function LanguagePicker({ onSelect }) {
  const CARDS = [
    {
      lang: 'SAS', color: '#f59e0b', dim: 'rgba(245,158,11,0.08)', glow: 'rgba(245,158,11,0.2)',
    ...
    {
      lang: 'PySpark', color: '#38bdf8', dim: 'rgba(56,189,248,0.08)', glow: 'rgba(56,189,248,0.2)',
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit', sans-serif", padding: '60px 20px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 72 }} className="fade-up">
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 99, padding: '6px 16px', marginBottom: 28, fontSize: 12, color: '#34d399', letterSpacing: 2, fontWeight: 600 }}>
        ...
        <div style={{ width: sidebarOpen ? 276 : 0, minWidth: sidebarOpen ? 276 : 0, overflow: 'hidden', flexShrink: 0, transition: 'width 0.38s cubic-bezier(0.4,0,0.2,1), min-width 0.38s cubic-bezier(0.4,0,0.2,1)' }}>
          <div style={{ width: 276, height: '100%', opacity: sidebarOpen ? 1 : 0, transform: sidebarOpen ? 'translateX(0)' : 'translateX(-18px)', transition: sidebarOpen ? 'opacity 0.32s cubic-bezier(0.4,0,0.2,1) 0.06s, transform 0.38s cubic-bezier(0.34,1.4,0.64,1) 0.04s' : 'opacity 0.18s, transform 0.22s', pointerEvents: sidebarOpen ? 'auto' : 'none' }}>
            <SidebarPanel
              ...
              sasDone={Object.keys(completedSAS).length}  pyDone={Object.keys(completedPySpark).length}
              sasPct={Math.round(Object.keys(completedSAS).length / totalLessons * 100)}
              pyPct={Math.round(Object.keys(completedPySpark).length / totalLessons * 100)}
            />
          </div>
        </div>
